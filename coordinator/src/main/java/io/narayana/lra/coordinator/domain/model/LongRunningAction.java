...
public class LongRunningAction extends BasicAction {
    ...
    private boolean restore_list(InputObjectState os, int ot, RecordList list) {
        ...
        try {
            while ((record_type = os.unpackInt()) != RecordType.NONE_RECORD) {
                // Bypass RecordTypeManager for LRA_RECORD: the WildFly module registers
                // its own LRAParticipantRecord first ("stop at first hit"), so create()
                // would return the module-classloader version whose this.lra is never set.
                // Instantiate directly so the instanceof check below always succeeds.
                AbstractRecord record = record_type == RecordType.LRA_RECORD
                        ? new LRAParticipantRecord()
                        : AbstractRecord.create(record_type);
    ...
    public boolean updateRecoveryURI(String linkHeader, String recoveryUri) {
        ...
        if (lraRecord != null) {
            try {
                ...
                lraRecord.setParticipantPath(linkHeader);
        ...
        } else if (updateParticipantViaReflection(linkHeader, recoveryUri)) {
            return deactivate();
        }
    ...
    private boolean updateParticipantViaReflection(String linkHeader, String recoveryUri) {
        try {
            String lookupPath = new URI(recoveryUri).getPath();
            if (lookupPath == null) {
                return false;
            }
            for (RecordList list : new RecordList[] { pendingList, preparedList, heuristicList, failedList }) {
                if (list != null) {
                    for (AbstractRecord rec = list.peekFront(); rec != null; rec = list.peekNext(rec)) {
                        if (!rec.getClass().getName().equals(LRAParticipantRecord.class.getName())) {
                            continue;
                        }
                        java.lang.reflect.Method getRecoveryURI = rec.getClass().getMethod("getRecoveryURI");
                        Object uri = getRecoveryURI.invoke(rec);
                        if (uri != null && lookupPath.equals(((URI) uri).getPath())) {
                            java.lang.reflect.Method setParticipantPath = rec.getClass().getDeclaredMethod("setParticipantPath",
                                    String.class);
                            setParticipantPath.setAccessible(true);
                            setParticipantPath.invoke(rec, linkHeader);
                            return true;
                        }
                    }
                }
            ...
            LRALogger.logger.warnf(e, "updateParticipantViaReflection failed");
        }
        return false;
    }
    ...
    public java.util.Map<String, String> getParticipantMap() {
            ...
            if (list != null) {
                for (AbstractRecord rec = list.peekFront(); rec != null; rec = list.peekNext(rec)) {
                    if (rec.getClass().getName().equals(LRAParticipantRecord.class.getName())) {
                        try {
                            java.lang.reflect.Method getRecoveryURI = rec.getClass().getMethod("getRecoveryURI");
                            java.lang.reflect.Method getParticipantURI = rec.getClass().getMethod("getParticipantURI");
                            Object uri = getRecoveryURI.invoke(rec);
                            Object participantUri = getParticipantURI.invoke(rec);
                            LRALogger.logger.infof("getParticipantMap: recoveryURI=%s, participantURI=%s", uri, participantUri);
                            if (uri != null && participantUri != null) {
                                result.put(uri.toString(), participantUri.toString());
                            }
                        } catch (Exception e) {
                            LRALogger.logger.warnf(e, "getParticipantMap: error extracting participant");
                        }
                    }
                }
            }
