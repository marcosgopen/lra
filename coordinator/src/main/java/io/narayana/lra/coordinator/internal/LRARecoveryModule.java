...
public class LRARecoveryModule implements RecoveryModule,
    ...
    /**
     * Recovers LRA transactions from the local filesystem ObjectStore.
     * In HA mode, cross-node participant data is in the Infinispan caches
     * (lra-active, lra-participants) — the ObjectStore only holds local entries
     * for disaster recovery after a full cluster restart.
     */
    ...
    private void doRecoverTransaction(Uid recoverUid) {
        ...
        try {
            ...
            if (lra.getId() == null) {
                com.arjuna.ats.arjuna.state.InputObjectState state = _recoveryStore.read_committed(recoverUid,
                        _transactionType);
                if (state != null) {
                    lra.restore_state(state, com.arjuna.ats.arjuna.ObjectType.ANDPERSISTENT);
                }
            }
