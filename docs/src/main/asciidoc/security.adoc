...
|`TRACE`
|"No active CDI request scope for JWT resolution: ..."
|No request scope on this thread -- expected on async, recovery, and callback threads;
resolution falls back to the client property (participant calls) or the service token
(coordinator recovery) instead of failing

|`DEBUG`
|"No CDI container available for JWT resolution: ..."
|No CDI container is present (e.g. a non-CDI runtime)
