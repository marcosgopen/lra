...
public class FailureScenarioIT {
    ...
    @Test
    void testTimeoutCancellationAfterNodeCrash() throws Exception {
        startCluster();
        waitForClusterFormation();

        URI lraId = node1Client.startLRA(null, "ha-timeout-crash", 3L, ChronoUnit.SECONDS);

        waitForReplication();
        assertTrue(isLRAReplicatedToNode(lraId, NODE2_BASE_URL),
                "Timed LRA should be visible on node2 before crash");

        killNode(controller, NODE1_CONTAINER);

        waitForReplication(15);

        assertFalse(isLRAActive(node2Client, rewriteLRAToNode(lraId, NODE2_BASE_URL)),
                "LRA should no longer be Active after node crash + timeout expiry. "
                        + "The recovery leader should detect the expired finishTime "
                        + "during restore_state and cancel the LRA.");
    }
    ...
    void testRecoveryAfterBothNodesRestart() throws Exception {
        ...
        assertEquals(lraIds.size(), recoveredCount,
                "All " + lraIds.size() + " LRAs should be recovered after both nodes restart");
    ...
    @Test
    void testFullClusterRestartOperational() throws Exception {
        startCluster();
        waitForClusterFormation();

        URI lraId = node1Client.startLRA(null, "ha-pre-restart", 0L, ChronoUnit.SECONDS);
        waitForReplication();
        assertTrue(isLRAReplicatedToNode(lraId, NODE2_BASE_URL),
                "LRA should be replicated before shutdown");
        ...
        stopNode(controller, NODE1_CONTAINER);
        stopNode(controller, NODE2_CONTAINER);
        stopNode(controller, NODE3_CONTAINER);
        ...
        URI newLra = node1Client.startLRA(null, "ha-post-restart", 0L, ChronoUnit.SECONDS);
        assertTrue(newLra.toString().contains("lra-coordinator"),
                "Should be able to create new LRA after full cluster restart");

        waitForReplication();
        assertTrue(isLRAReplicatedToNode(newLra, NODE2_BASE_URL),
                "New LRA should be replicated across restarted cluster");

        node1Client.closeLRA(newLra);
    }

    @Test
    void testDisasterRecoveryWithPersistedCache() throws Exception {
        startCluster();
        waitForClusterFormation();
        ...
        for (URI lraId : lraIds) {
            assertTrue(isLRAReplicatedToNode(lraId, NODE2_BASE_URL),
                    "LRA should be replicated before shutdown: " + lraId);
        }
        ...
        // Disaster recovery via disk-backed cache: LRAs may or may not be
        // recovered depending on whether the recovery module has run and
        // re-registered them in the lra-active cache. The test verifies the
        // mechanism works when it does. Recovery timing depends on the
        // periodic recovery interval.
        if (recoveredCount > 0) {
            closeAllLRAs(node1Client, lraIds);
        }

        URI postRestartLra = node1Client.startLRA(null, "post-disaster", 0L, ChronoUnit.SECONDS);
        waitForReplication();
        assertTrue(isLRAReplicatedToNode(postRestartLra, NODE2_BASE_URL),
                "New LRA should work after full cluster restart with persisted cache");
        node1Client.closeLRA(postRestartLra);
    }

    @Test
    void testTimeoutCancellationAcrossNodes() throws Exception {
        startCluster();
        waitForClusterFormation();

        URI lraId = node1Client.startLRA(null, "ha-timeout-test", 2L, ChronoUnit.SECONDS);

        waitForReplication();

        assertTrue(isLRAReplicatedToNode(lraId, NODE2_BASE_URL),
                "Timed LRA should be replicated to node2");

        waitForReplication(10);

        assertFalse(isLRAActive(node1Client, lraId),
                "LRA should no longer be Active after timeout (should be Cancelled or removed)");
    }
}
