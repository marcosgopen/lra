...
public class MultiNodeClusterIT {
    ...
    @Test
    void testCloseLRAFromDifferentNode() throws Exception {
        URI lraId = node1Client.startLRA(null, "ha-close-test", 0L, ChronoUnit.SECONDS);

        waitForReplication();

        assertTrue(isLRAReplicatedToNode(lraId, NODE2_BASE_URL),
                "LRA should be visible on node2 before close");
        ...
        assertFalse(isLRAReplicatedToNode(lraId, NODE2_BASE_URL),
                "LRA should no longer be listed as active on node2 after close");
        assertFalse(isLRAReplicatedToNode(lraId, NODE1_BASE_URL),
                "LRA should no longer be listed as active on node1 after close from node2");
    }

    @Test
    void testCancelLRAFromDifferentNode() throws Exception {
        URI lraId = node1Client.startLRA(null, "ha-cancel-test", 0L, ChronoUnit.SECONDS);

        waitForReplication();

        assertTrue(isLRAReplicatedToNode(lraId, NODE2_BASE_URL),
                "LRA should be visible on node2 before cancel");

        URI lraOnNode2 = rewriteLRAToNode(lraId, NODE2_BASE_URL);
        node2Client.cancelLRA(lraOnNode2);

        waitForReplication();

        assertFalse(isLRAReplicatedToNode(lraId, NODE2_BASE_URL),
                "LRA should no longer be listed as active on node2 after cancel");
        assertFalse(isLRAReplicatedToNode(lraId, NODE1_BASE_URL),
                "LRA should no longer be listed as active on node1 after cancel from node2");
    }
