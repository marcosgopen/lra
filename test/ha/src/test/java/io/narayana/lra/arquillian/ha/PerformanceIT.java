 ...
 * - Latency overhead (HA replicated cache vs single-node)
 * - Concurrent load handling under contention
 * - Cache memory limits
 * - Replication performance
 *
 * These tests verify that HA mode with replicated Infinispan cache
...
public class PerformanceIT {
    ...
    void testHighThroughputLRACreation() throws Exception {
        ...
        // HA target: ~8-18ms per LRA (single-node baseline ~7-15ms)
    ...
    void testClusterThroughputCapacity() throws Exception {
        ...
        try {
            ...
            // Capture submission window end and ops count before draining
            long submissionEndTime = System.currentTimeMillis();
            int opsInWindow = opsCompleted.get();
            ...
            // Calculate throughput using the submission window duration only
            double submissionDuration = (submissionEndTime - startTime) / 1000.0;
            double throughput = opsCompleted.get() / submissionDuration;

            assertTrue(throughput > 10,
                    String.format("Cluster should handle >10 ops/s, got %.2f ops/s "
                            + "(ops=%d, opsInWindow=%d, duration=%.2fs)",
                            throughput, opsCompleted.get(), opsInWindow, submissionDuration));
