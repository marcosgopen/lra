...
public class RecoveryCoordinatorHAIT {
    ...
    void testGetCompensatorFromDifferentNode() throws Exception {
        ...
        URI compensateUri = participantCompensateUri(NODE1_BASE_URL);
        URI completeUri = participantCompleteUri(NODE1_BASE_URL);
        ...
        try (Client httpClient = ClientBuilder.newClient()) {
            ...
            assertTrue(participantUrl.contains(SimpleParticipant.PATH),
                    "Response should contain the participant URL, but got: " + participantUrl);
        }
    ...
    void testReplaceCompensatorVisibleAcrossNodes() throws Exception {
        ...
        String newCompensatorLink = participantLink(NODE2_BASE_URL);
    ...
    void testCompensatorSurvivesNodeCrashAndUpdate() throws Exception {
        ...
        URI recoveryUrl = node1Client.joinLRA(lraId, 0L,
                compensateUri, completeUri, null, null, null, null, (String) null);
        assertTrue(recoveryUrl != null, "Join should return a recovery URL");
        ...
        String updatedCompensatorLink = participantLink(NODE2_BASE_URL);
    ...
    /**
     * Tests that a participant can re-enlist with the recovery coordinator
     * after the originating node restarts. The MicroProfile LRA spec requires
     * that the recovery endpoint remains available for participant re-enlistment.
     */
    @Test
    void testParticipantReEnlistmentAfterRestart() throws Exception {
        URI lraId = node1Client.startLRA(null, "recovery-reenlist-test", 0L, ChronoUnit.SECONDS);
        ...
        URI newCompensateUri = participantCompensateUri(NODE2_BASE_URL);
        URI newCompleteUri = participantCompleteUri(NODE2_BASE_URL);

        URI newRecoveryUrl = node1Client.joinLRA(lraId, 0L,
                newCompensateUri, newCompleteUri, null, null, null, null, (String) null);
        assertTrue(newRecoveryUrl != null,
                "Re-enlistment after coordinator restart should return a new recovery URL");
