...
public class TestHelpers {
    ...
    /**
     * Check whether an LRA is in Active status on the given client's coordinator.
     * Unlike {@link #isLRAAccessible}, this filters by Active status so that
     * closed/cancelled LRAs are not considered "active".
     */
    public static boolean isLRAActive(NarayanaLRAClient lraClient, URI lraId) {
        try {
    ...
    /**
     * Check whether an LRA is in Active status on a target node.
     */
    public static boolean isLRAActiveOnNode(URI lraId, String targetNodeBaseUrl) {
        try (NarayanaLRAClient targetClient = createLRAClient(targetNodeBaseUrl)) {
    ...
    /**
     * Rewrite an LRA URI so that it targets a different node's coordinator.
     * The NarayanaLRAClient extracts the coordinator URL from the LRA URI itself,
     * so after a node crash we must rewrite the URI to route through a surviving node.
     */
    public static URI rewriteLRAToNode(URI lraId, String targetNodeBaseUrl) {
        String uid = LRAConstants.getLRAUid(lraId);
        return URI.create(targetNodeBaseUrl + LRA_COORDINATOR_PATH + "/" + uid);
    }
    ...
    public static WebArchive createLRACoordinatorDeployment() {
        ...
        return ShrinkWrap.create(WebArchive.class, "lra-coordinator.war")
                ...
                .addAsLibraries(Maven.configureResolver()
                        ...
                        .withoutTransitivity().asFile())
                .addClass(SimpleParticipant.class)
    ...
    /**
     * Returns the participant link header for a SimpleParticipant deployed on the given node.
     * The link header format follows MicroProfile LRA specification.
     */
    public static String participantLink(String nodeBaseUrl) {
        String base = nodeBaseUrl + "/lra-coordinator/" + SimpleParticipant.PATH;
        return "<" + base + "/compensate>;rel=\"compensate\","
                + "<" + base + "/complete>;rel=\"complete\"";
    }

    public static URI participantCompensateUri(String nodeBaseUrl) {
        return URI.create(nodeBaseUrl + "/lra-coordinator/" + SimpleParticipant.PATH + "/compensate");
    }

    public static URI participantCompleteUri(String nodeBaseUrl) {
        return URI.create(nodeBaseUrl + "/lra-coordinator/" + SimpleParticipant.PATH + "/complete");
    }
